...
{
    /// <summary>
    /// Draws the on-screen detection indicator and reacts to state changes from the detection system.
    /// </summary>
    public sealed class HUDManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Binds the manager to the detection system and registers change notifications.
        /// </summary>
        public void Initialize(DetectionSystem system)
        {
            detectionSystem = system;
            if (detectionSystem != null)
            {
                detectionSystem.DetectionStateChanged += OnDetectionStateChanged;
            }
        }
        ...
        /// <summary>
        /// Updates the cached zombie count for informational purposes.
        /// </summary>
        ...
        private void OnGUI()
        {
            if (!ModConfig.ShowHud || detectionSystem == null)
            {
                return;
            }
            ...
            if (Time.time - lastHudUpdate < ModConfig.HudUpdateInterval)
            {
                return;
            }
        ...
        private void DrawHud()
        {
            ...
            detectionStyle.fontSize = Mathf.RoundToInt(ModConfig.FontSize * scale);
            detectionStyle.normal.textColor = currentColor;
            infoStyle.fontSize = Mathf.RoundToInt(12 * scale);

            var offset = ModConfig.HudOffset;
            ...
            DrawLabel(textRect, currentStatus, detectionStyle, ModConfig.UseOutline);
