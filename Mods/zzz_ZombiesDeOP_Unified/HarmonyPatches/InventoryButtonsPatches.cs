...
{
    [HarmonyPatch]
    public static class InventoryButtonsPatches
    {
        ...
        private static void OnGameManagerAwake()
        {
            ...
            ModLogger.Log("Inicializando módulo de botones de inventario");
        }
    ...
    internal static class InventoryButtonsManager
    {
        ...
        internal static void TryInjectButtons(object windowGroup)
        {
            ...
            if (!InventoryEnhancementConfig.EnableSortButton && !InventoryEnhancementConfig.EnableLootAllButton)
            {
                return;
            }
            ...
            try
            {
                ...
                ModLogger.Log($"Botones inyectados en ventana '{identifier}'.");
            }
            catch (Exception exception)
            {
                ModLogger.Warning($"Error al inyectar botones en '{identifier}': {exception.Message}");
            }
        }
        ...
        private static void InjectButtonSet(object windowGroup, bool isInventory, bool isLoot)
        {
            ...
            {
                ModLogger.Warning("No se encontró un botón base para clonar dentro de la ventana destino.");
                return;
            }
            ...
            {
                ModLogger.Warning("El botón base carece de Transform válido, no se puede continuar.");
                return;
            }
            ...
            {
                ModLogger.Warning("No se pudo resolver el contenedor de botones en la ventana objetivo.");
                return;
            }

            int insertionIndex = templateTransform.GetSiblingIndex() + 1;

            if (isInventory && InventoryEnhancementConfig.EnableSortButton)
            {
            ...
            if (isLoot && InventoryEnhancementConfig.EnableLootAllButton)
            {
        ...
        private static void RegisterButtonCallback(object controller, string methodName)
        {
            ...
            if (eventInfo == null)
            {
                ModLogger.Warning($"El controlador {type.Name} no expone el evento OnPress.");
                return;
            }
        ...
        private static void TryRefreshWindow(object windowGroup)
        {
            ...
            if (refreshMethod != null)
            {
                ...
                {
                    ModLogger.Debug($"RefreshBindings falló: {exception.Message}");
                }
            }
        }
        ...
        {
            if (!isDown || !InventoryEnhancementConfig.EnableSortButton)
            {
                return;
            }
            ...
            {
                ModLogger.Warning($"Fallo al ordenar inventario: {exception.Message}");
            }
        }

        private static void OnLootAllButtonPressed(object controller, int mouseButton, bool isDown)
        {
            if (!isDown || !InventoryEnhancementConfig.EnableLootAllButton)
            {
                return;
            }
            ...
            {
                ModLogger.Warning($"Fallo al saquear contenedor: {exception.Message}");
            }
        }
        ...
        private static void ExecuteSort(object controller)
        {
            ...
            if (playerUI == null)
            {
                ModLogger.Warning("No se pudo localizar LocalPlayerUI para ordenar el inventario.");
                return;
            }
            ...
            if (xui == null)
            {
                ModLogger.Warning("No se pudo resolver la instancia de XUi desde LocalPlayerUI.");
                return;
            }
            ...
            if (bagWindow == null)
            {
                ModLogger.Warning("No se encontró la ventana de inventario en XUi.");
                return;
            }

            if (InvokeMethodAny(bagWindow, new[] { "OnSortClicked", "OnSortPressed", "SortInventory", "SortItems", "Sort" }))
            {
                ModLogger.Debug("Ordenación delegada al controlador de inventario.");
                return;
            }
            ...
            {
                ModLogger.Debug("Ordenación ejecutada a través de ItemStackGrid.");
                return;
            }

            ModLogger.Warning("No se encontraron métodos de ordenación disponibles en la ventana de inventario.");
        }
        ...
        private static void ExecuteLootAll(object controller)
        {
            ...
            if (playerUI == null)
            {
                ModLogger.Warning("No se pudo localizar LocalPlayerUI para saquear.");
                return;
            }
            ...
            if (lootWindow == null)
            {
                ModLogger.Warning("No se encontró la ventana de saqueo activa.");
                return;
            }

            if (InvokeMethodAny(lootWindow, new[] { "OnTakeAllPressed", "TakeAll", "TakeAllItems", "LootAll" }))
            {
                ModLogger.Debug("Saqueo completo ejecutado mediante la ventana de loot.");
                return;
            }
            ...
            {
                ModLogger.Debug("Saqueo completo ejecutado mediante el controlador de grid.");
                return;
            }

            ModLogger.Warning("No se encontraron métodos TakeAll compatibles en la ventana de loot.");
        }
        ...
        private static bool InvokeMethodAny(object instance, IEnumerable<string> methodNames)
        {
            ...
            foreach (string methodName in methodNames)
            {
                ...
                {
                    ModLogger.Debug($"Invocation {methodName} fallida: {exception.Message}");
                }
            }
